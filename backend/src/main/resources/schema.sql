...
CREATE TABLE
    IF NOT EXISTS weapon_template (
        ...
        crit_multiplier DOUBLE PRECISION DEFAULT 1.0,
...
CREATE TABLE
    IF NOT EXISTS user_weapon_instance (
        id SERIAL PRIMARY KEY,
...
CREATE TABLE
    IF NOT EXISTS user_stats_summary (
...
CREATE TABLE
    IF NOT EXISTS match_player_stats (
...
CREATE TABLE
    IF NOT EXISTS match_weapon_stats (
        id SERIAL PRIMARY KEY,
        match_id INTEGER NOT NULL REFERENCES match_state (id) ON DELETE CASCADE,
        user_id INTEGER NOT NULL REFERENCES app_user (id) ON DELETE CASCADE,
        ...
        kills INTEGER DEFAULT 0,
        crits_landed INTEGER DEFAULT 0,
        CONSTRAINT unique_match_user_weapon UNIQUE (
            match_id,
            user_id,
            template_id,
            user_weapon_instance_id
        )
    );
